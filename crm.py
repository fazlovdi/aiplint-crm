...
# --- Навигация (Сделки и Клиенты поменяны местами) ---
    ...
    def render_task_block(t, sk):
        ...
        with st.expander(ht, expanded=False, key=exp_key):
            ...
            if task.get("file_path"):
                ...
                render_file_action_buttons(task["file_path"], task.get("file_name", ""), f"task_{sk}_{t['client_id']}_{t['task_idx']}", allow_delete=(st.session_state.user_role == "admin"))
                if st.session_state.user_role == "admin":
            ...
            if client.get("comments"):
                with st.expander("Отчёты по задачам"):
                    for com in client["comments"]:
                        ...
                        if com.get("file_path"): render_file_action_buttons(com["file_path"], com.get("file_name", "файл"), f"dcom_{deal['id']}_{com['time'].replace(':','_')}", allow_delete=(st.session_state.user_role == "admin"))
            st.markdown("**Задачи:**")
            if client.get("tasks"):
                ...
                for orig_i, task in sorted_indexed:
                    ...
                    with st.expander(ht2, expanded=False, key=task_exp_key):
                        if task.get("done"): st.markdown(f"~~{task['text']}~~ — выполнено")
                            ...
                            if task.get("file_path"):
                                st.markdown(f"📄 {task.get('file_name', '')}")
                                render_file_action_buttons(task["file_path"], task.get("file_name", ""), f"dtk_{deal['id']}_{orig_i}", allow_delete=(st.session_state.user_role == "admin"))
                                if st.session_state.user_role == "admin":
...
# --- Вкладка «Клиенты» ---
            ...
            with st.expander(f"{cl['name']} — ID: {cl['id']} [{cl.get('category', 'Покупатель')}]", expanded=itc, key=client_exp_key):
                ...
                with cl_l:
                    ...
                    for cfi, cf in enumerate(cl["client_files"]):
                        ...
                        render_file_action_buttons(cf.get("file_path"), cf.get("file_name", "файл"), f"cli_{cl['id']}_{cfi}", allow_delete=(st.session_state.user_role == "admin"))
                        if st.session_state.user_role == "admin":
    ...
    else: st.info("База клиентов пуста. Создайте первого клиента.")def get_task_sort_date(task):
    dl = task.get("deadline", "")
    try: return datetime.strptime(dl, "%Y-%m-%d").date()
    except:
        try: return datetime.strptime(dl, "%Y-%m-%d %H:%M").date()
            ...
            with st.expander(f"{cl['name']} — ID: {cl['id']} [{cl.get('category', 'Покупатель')}]", expanded=itc, key=client_exp_key):
                ...
                with cl_r:
                    ...
                    if st.button("Создать сделку", key=f"dbn_{cl['id']}", use_container_width=True, type="primary"):
                        ...
                        st.session_state.open_deal_id = ndi; st.session_state.active_tab = "Сделки"; st.rerun()
            if st.session_state.get("last_id"): st.session_state.last_id = None
    else: st.info("База клиентов пуста. Создайте первого клиента.")
